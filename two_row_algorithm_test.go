...
func TestLevenshteinDistanceTwoRows(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if got := LevenshteinDistanceTwoRows(tc.a, tc.b); got != tc.wanted {
				t.Errorf("wanted %d, got %d", tc.wanted, got)
			}
			if got := LevenshteinDistanceTwoRows(tc.b, tc.a); got != tc.wanted {
...
func BenchmarkLevenshteinDistanceTwoRows(b *testing.B) {
	b.Run("short", func(b *testing.B) {
		for b.Loop() {
			if got := LevenshteinDistanceTwoRows("abc", "def"); got == 0 {
	...
	b.Run("medium", func(b *testing.B) {
		for b.Loop() {
			if got := LevenshteinDistanceTwoRows("abcedf", "defabc"); got == 0 {
				b.Error("non-zero distance")
			}
		}
	})
	b.Run("longer", func(b *testing.B) {
		for b.Loop() {
			if got := LevenshteinDistanceTwoRows("abcedfabcedf", "vbcedfabcedf"); got == 0 {
				b.Error("non-zero distance")
			}
		}
	})
